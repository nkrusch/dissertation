...
Formal verification\index{formal verification} aims to guarantee that certain behavioral attributes, \ie \emph{properties}, hold for a program.
...
Total correctness\index{total correctness} can be viewed as a combination of safety\index{safety property} and liveness properties\index{liveness property}~\cite{lamport1977}.
...
\footnote{Total correctness\index{total correctness} is not always ideal.
...
Interactive theorem provers\index{interactive proof assistant} and
...
\item An \emph{interactive theorem prover}\index{interactive proof assistant}
(ITP), \aka \emph{interactive proof assistant}, is a programming environment
that helps users write \ndx{mechanized proof}s. Proof assistants vary in
...
assistants with highest coverage of the problems are \ndx{Isabelle} (92\%),
...
tools. Interactive theorem provers\index{interactive proof assistant} enable
...
The \ndx{Rocq} theorem prover is an \ndx{interactive proof assistant} for
...
dependent types\index{dependent type theory} and \ndx{type
...
annotate the program with additional guidance---assertions, \ndx{precondition}s,
...
without a proof; and \ndx{ghost method}s, \ie lemmas that are not yet proven.
...
In Dafny, the \ndx{specifications} are \enquote{embedded} with the implementation.
...
The \pr|decreases| clause proves termination, which established \ndx{total correctness} of the \pr|LeftPad| method.
