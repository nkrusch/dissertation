...
requirements~\cite{terbeek2018}. Two other common terms are \emph{\ndx{safety}}
and \emph{\ndx{liveness}}. They come from a paper by~\textcite{lamport1977}
where they are formulated as follows. A \emph{safety
property}\index{properties!safety} is a guarantee that \enquote{something (bad)
will not happen}. A \emph{liveness property}\index{properties!liveness} ensures
\enquote{(good) things must happen}, eventually. The proof techniques across
these safety and liveness properties are different. The works in this
...
\index{interactive proof assistant}\\
&
