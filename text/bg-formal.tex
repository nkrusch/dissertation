...
One standard way to express \ndx{specifications} is by \ndx{Hoare
triple}s~\cite{hoare1969}. In this view, we consider \(S\) to be some executable
...
satisfying \(Q\)\symbo{post}}. For example, \(\{ \prm{x}=0 \}\;\prm{x++}\;\{
\prm{x}=1\}\)\symbo{hoare} is a valid and provable\footnote{ Though, the
...
and \emph{\ndx{liveness property}} ensures (good things) \enquote{must happen}, eventually.
...
\\
   ...
   \smtabularnote{CompCert received the ACM Software System Award in 2021.}
   & \textcite{leroy2009}
\\
...
However, the effort of formalizing ICC results is justified, based on the following factors.
\begin{enumerate*}
\item Continued demonstrated relevance of ICC results -- \cf~\autoref{tab:icc-results}.
\item Absence of \emph{formal} proofs among those results.
\item Maturity of formalization techniques and tools.
\end{enumerate*}

\paragraph*{Existing results in formally verified complexity.}
There exists a (small) collection of result formalizing classic complexity
...
and the \ndx{Cook-Levin theorem}~\cite{gaher2021}%! suppress = CollapseCite
\footnote{The Cook-Levin theorem states that Boolean SAT is
NP\ccxi{np}-Complete~\cite{cook1971, levin1973}.}
have been formalized previously.

Among programming language-based approaches, the type system of~\cite{crary2000}
is notable. The type system assigns to programs \enquote{virtual clocks}, and
then checks that the clock does not expire. Although this provides an automatic
method for certifying resource usage, the theory is not formalized mechanically.
\ndx{Calf}~\cite{niu2022} (and its extension \ndx{Decalf}~\cite{grodin2024}) is
similarly based on type systems. Calf embeds, through types, the amortized costs
\index{amortization} of programs to record quantitative aspects of functional
programs. Then, if a program can be typed, Calf enables proving that the program
satisfies specified resource bounds. The type system definitions is
...
Motivated by the various demonstrated applications, the flow calculus of
mwp-bounds is an appealing target for formalization. An important step is
proving formally the mwp-calculus soundness theorem (\autoref{mwp-soundness}).
Such pursuit benefits from the facts that (i) the paper proofs are available
in~\cite{jones2009}, and (ii) our confidence in the theory being correct is high
(\ie if a problem existed, it likely would have surfaced in the prior projects).
However, converting the mwp-calculus to a formal proof---and in Rocq this means
converting from set theory to type theory---poses various challenges. Further,
such proof would not extend to enhanced \textsc{mwp}$^\infty$-calculus variant
of the calculus that is used in program analysis. Yet, starting with the core
mwp-calculus and paper proofs is a solid strategy toward formalizing complexity.
The result would extend broadly to all programs that share the same imperative
core. This project idea is discussed in~\autoref{sec:mwp-calc-formal}.
...
idea is that inferred mwp-bounds can be used as postconditions in
