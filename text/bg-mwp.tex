...
\begin{theorem}[{Soundness}]\label{mwp-soundness}
...
\begin{lstlisting}[language=C,numbers=none,caption={A derivable program.}]
X1=X2+X3; X1=X1+X1
\end{lstlisting}
...
\begin{lstlisting}[language=C,numbers=none,caption={A loop that always fails the derivation.}]
loop X2 { X1=X1+X1 }
\end{lstlisting}
...
\begin{lstlisting}[language=C,numbers=none,caption={A loop that sometimes fails the derivation.}]
loop X3 { X2=X1+X2 }
\end{lstlisting}
...
\begin{lstlisting}[language=C,numbers=none,caption={Exponential program, exponent\_2.c.}]
while(i<e) { r=r*b; i=i+1; }
\end{lstlisting}
The derivation proceeds sequentially, starting with the loop body\index{loop body}.
...
\begin{lstlisting}[language=C,caption={While loop analysis, notinfinite\_3.c.}]
if(X1==1) {
  X1=X2+X1; X2=X3+X2;
}
while(X0<10)
...
As the name hints, and the tool user guide confirms, the program is derivable\index{derivability}.
...
\begin{minipage}{\textwidth}
\begin{lstlisting}[language=C,caption={An infinite program, infinite\_3.c.}]
if(X1==1) {
  X1=X2+X1; X2=X3+X2;
}
while(X0<10)
...
\begin{minipage}{\textwidth}
\begin{lstlisting}[language=C,escapeinside=||,caption={A challenge program.}]
if(X0)          |\circled{C0}|
  X2=X0+X1;
else
  X2=X2+X1;
