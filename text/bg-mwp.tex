...
\begin{center}
\lstinline[language=IMP]|X1=X2+X3; X1=X1+X1|
\end{center}
...
\begin{center}
\lstinline[language=IMP]|X1=1; loop X2 {X1=X1+X1}|
\end{center}
...
\index{mwp-calculus!soundness theorem}\symbo{cmat}\symbo{cmatsem}
...
Phase
    & Description
    & \textbf{flow calculus}
    & \textbf{\textsc{mwp}\(^\infty\) calculus}
    & \textbf{static analyzer}
    & \textbf{via mwp-bounds}
\\
\midrule
& Motivation
    & complexity theory
...
\textsc{input}
& Language
    & \(\mathcal{L}\) (baseline)\tabularnote{A language is similar to IMP in~\textcite{nielson2010,cpierce20221}.}
    & \(\mathcal{L} + f +\) recursion
    & \(\mathcal{L} \mapsto \) C99
    & loops \(\subset \mathcal{L}\) \\
\textsc{analysis}
& Calculus variant
    & \textsc{mwp}
    & \textsc{mwp}\(^\infty\)
...
& Generated bound
    & arbitrary
    & exists (y/n)\tabularnote{Determines if a bound exists, but does not provide bounds by default.}
    & arbitrary
    & optimal \\
& Bounds granularity
    & whole-program
...
The language and semantics are defined in~\autoref{sec:postcond} and~\aref{sec:fscd} thus not repeated here.
...
\end{pNiceMatrix}$
\end{center}
\caption[Variables labeling of an mwp-matrix]{Variables labeling of an mwp-matrix\index{mwp-matrix}.}
\label{fig:labels}
\end{figure}
...
\lstinputlisting[language=IMP,caption={[Derivable program]A derivable program.},label={lst:ex-derivable}]{derivable.c}
...
%! suppress = FileNotFound
\lstinputlisting[language=IMP,caption={[Loop that always fails the derivation]A loop that always fails the derivation.},label={lst:ex-failing}]{failing.c}
...
%! suppress = FileNotFound
\lstinputlisting[language=IMP,caption={[Loop that fails for some derivation paths]A loop that fails for some derivation paths.},label={lst:partial-fail}]{partial.c}
...
%! suppress = FileNotFound
\lstinputlisting[language=C,caption={[Program with a single assignment]A program with a single assignment.},label={lst:ex-assign}]{assign_expression.c}
\end{minipage}
...
\begin{minipage}{\textwidth}
%! suppress = FileNotFound
\lstinputlisting[language=C,caption={[Program with exponential variable value growth]A program with exponential variable value growth.},label={lst:ex-exponent2}]{exponent2.c}
\end{minipage}
...
\begin{minipage}{\textwidth}
%! suppress = FileNotFound
\lstinputlisting[language=C,caption={[Program with a \pr|while| loop]A program with a \pr|while| loop.},label={lst:notinfinite3}]{notinfinite3.c}
\end{minipage}
...
\begin{minipage}{\textwidth}
%! suppress = FileNotFound
\lstinputlisting[language=C,caption={[\enquote{Infinite} program]An \enquote{infinite} program.},label={lst:ex-infinite3}]{infinite3.c}
\end{minipage}
...
caption={[Challenge program whose derivability is unknown]A challenge program whose derivability is unknown.},
