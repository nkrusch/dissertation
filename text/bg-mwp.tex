...
that hold whenever \(\sem{\prm{C}}(x_i \rightsquigarrow x^\prime_i)\)\symbo{flowrel}\symbo{p2}.
...
The polynomially bounded data flow relation\index{data flow relation} \(\sem{\prm{C}}(x_1,x_2,x_3 \rightsquigarrow x^\prime_1,x^\prime_2,x^\prime_3)\)\symbo{flowrel}\symbo{p2}
...
\index{soundness of mwp-calculus}\symbo{cmat}\symbo{cmatsem}\symbo{vdash}\symbo{vsem}
\(\vdash \prm{C} : M \text{ implies } \vDash \prm{C} : M \).
\end{theorem}

The soundness theorem connects the syntactic data flow relation\index{data flow
relation} to the \emph{semantic property} \(\vDash \prm{C} : M\)\symbo{cmatsem}.
Informally, \ndx{derivability} guarantees that the variable values of program
\pr|C|\symbo{p2} will grow at most polynomially at runtime. This in turn ensures
program \pr|C|\symbo{p2} runs in polynomial time\ccxi{p}~\cite{kristiansen2017}.
The main result of the work introducing the flow
calculus\index{mwp-calculus}~\cite{jones2009} is precisely a proof of the
soundness theorem.\index{soundness of mwp-calculus} Since the analysis omits
\ndx{termination}, the flow analysis\index{mwp-calculus} provides a
\emph{\ndx{partial correctness}} guarantee.

The flow calculus\index{mwp-calculus} offers no guarantee to programs that are
not derivable\index{derivability}. Then, variable value growth is interpreted as
unknown. A program always fails if a variable value grows \enquote{too fast},
for example exponentially. A single variable can be the source of whole-program
\ndx{derivation failure}. Alternatively, failure may occur from inability to
express satisfiable behavior. The latter is a built-in limitation of the flow
calculus\index{mwp-calculus}. To increase \ndx{expressiveness} and capture a
larger class of derivable\index{derivability} programs, the \ndx{mwp-calculus}
includes \ndx{nondeterminism} in its \ndx{inference rule}s. Consequently, a
single program may admit multiple derivations with distinct
matrices\index{mwp-matrix}, which in turn complicates determining the result of
the analysis. However, a program is derivable\index{derivability} if there
exists a derivation without an \(\infty\)\symbo{infty}-coefficient.

\paragraph*{The mwp-bounds characterize value growth}\label{interpreting-bounds}
The column vectors of an mwp-matrix\index{mwp-matrix} encode variable value
growth bounds. An \ndx{mwp-bound} (denoted \(W,V,U,\ldots\))\symbo{bound}
characterizes the value growth of a single variable \wrt program inputs. An
\ndx{mwp-bound} is an expression of form $\max(\vec{x}, \poly_1(\vec{y})) +
\poly_2(\vec{z})$\symbo{vlist}. Variables characterized by $m$\symbo{m}-flow are
listed in $\vec{x}$\symbo{vlist}; $w$\symbo{w}-flows in $\vec{y}$\symbo{vlist},
and $p$\symbo{p}-flows in $\vec{z}$\symbo{vlist}. The notation
\(W(\vec{x};\vec{y};\vec{z})\)\symbo{bound}\symbo{vlist} displays all the
variables in an \ndx{mwp-bound} $W$\symbo{bound} where $\vec{x}$, $\vec{y}$ and
$\vec{z}$\symbo{vlist} are respectively the $m$-, $w$- and $p$-variables of
\(W\)\symbo{bound}. Variables characterized by $0$\symbo{zero}-flow do not occur
in the expression and no bound exists if some variable is characterized by
$\infty$\symbo{infty}. The $\poly_1$\symbo{hp} and $\poly_2$\symbo{hp} are
\textit{\ndx{honest polynomial}s}, build up from constants and variables by
applying $+$ and $\times$. Any of the three variable lists might be empty, and
$\poly_1$\symbo{hp} and $\poly_2$\symbo{hp} may not be present. When
characterizing value growth, an \ndx{mwp-bound} is approximative; it excludes
precise constants and degrees of polynomials. We write an \ndx{mwp-bound} of
variable \pr|X| as \(x' \leq W\) where \(x'\)\symbo{xprime2} is the final value
of \pr|X| and \(W\)\symbo{bound} is an \ndx{mwp-bound}. A \emph{program bound}
is a conjunction (\(\land\)) of its variables' \ndx{mwp-bound}s.

\begin{example}[Interpreting mwp-bounds]
For an arbitrary variable, the \ndx{mwp-bound} indicates the final value of the
variable is bounded by\ldots
...
\subsubsection{Advancements of the Flow Calculus, Chronologically}

The incremental technical advancement of the flow calculus of
\ndx{mwp-bound}s\index{mwp-calculus} is an important theme of this dissertation.
The developments are a necessary prerequisite for enabling different
\emph{applications} of the flow calculus.

It may be challenging to extract the \enquote{big picture of developments} by
reading the manuscripts alone. Therefore, this section aims discuss the topic in
an accessible manner, with a summary in~\autoref{tab:evo}).
The table corresponds to the following four publications.
...
pycparser converts the program source code into an \ndx{abstract syntax tree} (AST).
Before the inference, the AST\index{abstract syntax tree} is checked for consistency with the imperative language.
...
pymwp then runs the flow calculus of mwp-bounds on the (compatible) AST\index{abstract syntax tree}\@.
...
\textsc{input}
  & Language & \(\mathcal{L}\) (baseline)\smtabularnote{The language is similar to IMP\index{IMP language} in~\textcite[p. 11--26]{winskel1993} and~\textcite{cpierce20221}.} & \(\mathcal{L} + f +\) recursion & \(\mathcal{L} \mapsto \) C99 & loops \(\subset \mathcal{L}\) \\
\midrule
\textsc{procedure}
...
In the original mwp-calculus\index{mwp-calculus}, the problem of determining program derivability\index{derivability} is NP-complete.\index{\textsc{np}-completeness}
...
\item enhancing the \ndx{precision} of the flow calculus\index{mwp-calculus}.
\end{enumerate*}
...
\paragraph*{The \ndx{mwp-matrix} algebra}\index{mwp-matrix}
...
\begin{figure}[p!]
    ...
    \begin{subfigure}{\textwidth}
        ...
        \label{fig:mwp-rules-expressions}
    \end{subfigure}
    \\[3em]
    \begin{subfigure}{\textwidth}
        \begin{centering}
            \begin{tabularx}{.9\textwidth}{lCR}
            \begin{prooftree}[small,center=false]
                \infer0[S]{ \vdashJK \text{\pr|skip|} : \mathbf{1} }
            \end{prooftree}
            &
            \raisebox{-.1\height}{
            \begin{prooftree}[small,center=false,rule margin=0ex]
            \hypo{\vdashJK \text{\pr{e}} : V_{1}}
                \infer1[A]{\vdashJK\text{\pr|Xj = e|} : \mathbf{1}\xleftarrow{\scalebox{.6}{\pr|j|}} V
            }
            \end{prooftree}}
            &
            \begin{prooftree}[small,center=false]
    ...
    {The nondeterministic inference rules of the original \ndx{mwp-calculus} of~\textcite{jones2009}.}
    \label{fig:jkrules}\index{inference rule}\index{nondeterminism}\index{mwp-matrix}
\end{figure}

\begin{description}
\item[Expressions.]
An \emph{expression} \pr|e| is assigned a vector \(V\) of coefficients\index{flow coefficient} \(\prm{e} : V\).
The rules for analysing expressions, E1--E4, are defined in~\autoref{fig:mwp-rules-expressions}.
...
but that occur in realistic programming languages, \eg \ndx{assertion}s.
...
In practice, the analysis is performed over the program's \ndx{abstract syntax tree}.
...
Conceptually, the program analysis is straightforward: compose the mwp-matrices of commands in the order they appear in the AST\index{abstract syntax tree}\@.
...
\captionof{lstlisting}[Transformed program with arithmetic]{Program with arithmetic (\(\mathsf{P_A}\)), after transformation.}
