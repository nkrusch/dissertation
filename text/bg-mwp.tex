...
\textsc{input}
  & Language & \(\mathcal{L}\) (baseline)\smtabularnote{A language is similar to IMP in~\textcite{nielson2010,cpierce20221}.} & \(\mathcal{L} + f +\) recursion & \(\mathcal{L} \mapsto \) C99 & loops \(\subset \mathcal{L}\) \\
\textsc{analysis}
...
\begin{minipage}{\textwidth}
%! suppress = FileNotFound
\lstinputlisting[language=C,caption={[Program with a \texttt{while} loop]A program with a \texttt{while} loop.},label={lst:notinfinite3}]{notinfinite3.c}
\end{minipage}
