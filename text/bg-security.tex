Computer security is a broad, interdisciplinary field with technical and
...
\ndx{integrity}, and \ndx{availability}~\cite{du2019}. Although the precise
...
The goal of \ndx{confidentiality} is to ensure protected assets are viewed by
...
exposed, or \enquote{\ndx{leak|seealso{violation}}}, to unauthorized users.
...
Authentication\index{authentication} is a critical (though an insufficient)
mechanism to enforce \ndx{integrity}. Integrity is often considered the dual of
...
Confidentiality\index{confidentiality} can be enforced with numerous mechanisms,
\eg \ndx{firewall}s, \ndx{access control}, and \ndx{encryption}. A
\ndx{firewall} is a network-layer protection, and an operating system implements
and maintains \ndx{access control}. Encryption\index{encryption} protects secret
information until \ndx{decryption}, but its excessive use is problematic for
\ndx{availability}. In general, those mechanisms are insufficient to provide
end-to-end guarantees through all layers of computation. One problem is that
\ndx{confidentiality} is not guaranteed after secret information is released as
input to a program~\cite{zdancewic2004}. Information flow\index{information
flow} security is concerned with achieving fine-grained \ndx{confidentiality}
(and \ndx{integrity}) guarantees by tracking information propagation during
program executions~\cite{hedin2012,eggert2014}.

\subsubsection{Information Flow Security}
\label{if-security}

Protected assets may require different levels of \ndx{confidentiality}. Computer
systems that protect assets at different security levels need controlled
\ndx{information flow}. The capability to process assets at different security
levels is called {\emph{\ndx{multilevel security}} (MLS)}~\cite{bossi2005}.
...
\ndx{multilevel security} system~\cite{feiertag1977}. The permissions cascade:
...
The main idea for achieving secure multilevel\index{multilevel security}
\ndx{information flow} is to impose constraints on how privileged information is
permitted to flow within the system. Secure \ndx{information flow} is a
\ndx{confidentiality} guarantee, because it aims to ensure secrets do not
\ndx{leak} to unauthorized parties~\cite{piessens2024}. Implementing secure
\ndx{information flow} requires three steps~\cite{eggert2014}.

\begin{enumerate} 

\item Identifying \emph{\ndx{security class}es} (alternatively, domains or user groups).
...
For strong \ndx{confidentiality} guarantees, the \ndx{information flow} approach
should also consider and protect against \ndx{side channel}s. \emph{Side
...
channels are \ndx{termination}~\cite{stefan2012}, divergence, \ndx{deadlocking},
...
then there exists an \ndx{information flow} from \({A}\) to \({B}\).
...
\begin{center}
 ...
 \captionof{lstlisting}[High conditional incremental leak]{
  ...
  implicitly\index{information flow!implicit} the value of the secret variable
  \pr|h|\symbo{high} through variable \pr|l|\symbo{low}. The \ndx{leak} reveals
  every value of \pr|h| precisely. In the secure version, value of \pr|h| cannot
  be deduced. But even the secure version may be vulnerable over a \ndx{side
  channel}, if the loop execution time is observable to an attacker.
  \index{attacker (adversary)}
...
\item System model\index{system model}: a precise mathematical definition of the system to be secured.

\item Security objective\index{security objective} (or properties\index{security property}):
a specification of system behaviors that are considered secure.

\item Attack model\index{attack model} (or \ndx{threat model}):
...
{non-interfering\index{non-interference}} if {secret (high) inputs} do not
affect the calculation of {public (low) outputs}~\cite{sabelfeld2003}. The idea
to use programming languages to model {non-interference\index{non-interference}}
was introduced by~\textcite{volpano1996}. The paper shows that
\ndx{non-interference} can be soundly\index{soundness} approximated using a
{\ndx{security type system}}. The influential result has since then been
followed by numerous works in language-based security. Among a few examples,
\ndx{non-interference} has been studied in terms of \ndx{static program
analysis}~\cite{barthe2007,huang2014}, \ndx{secure
compilation}~\cite{patrignani2017,myers1999,cecchetti2017}, \ndx{formal
methods}~\cite{kammuller2008,nelson2020}, program
logics~\cite{frumin2021,karbyshev2018,garg2006,beringer2007},
verification~\cite{eilers2023}, and \ndx{software testing}~\cite{hritcu2013}. In
general, evaluating \ndx{non-interference} over at different levels of
...
{non-interfering\index{non-interference}} \wrt its syntax, it is not protected
against vulnerabilities occurring at lower abstraction
levels~\cite{piessens2024}.

To develop intuition of programming languages based \ndx{non-interference}, the
rest of this subsection shows examples using {a \ndx{security type system}}
...
For a programming language, we consider the \ndx{grammar} shown in
~\autoref{fig:ni-syntax}. A program (command) in this language is denoted by
\pr|C|\symbo{p2}. Computation starts in an input state with high and low
variables \(s = (s_h, s_l)\)\symbo{state0}\symbo{low}\symbo{high}. Either
\pr|C|\symbo{p2} terminates in an output state with final values \(s' = (s_h',
s_l')\)\symbo{state1}\symbo{low}\symbo{high}, or diverges, which we denote by
\(\bot\)\symbo{diver}. Letting \(S\)\symbo{states} be the set of input states,
\(s \in S \)\symbo{state0}, the semantics \(\sem{\text{\pr|C|}}\)\symbo{funprog}
of \pr|C|\symbo{p2} is defined by a function \(\sem{\text{\pr|C|}} : S
\rightarrow S_\bot \)\symbo{funprog}\symbo{states}\symbo{diver} where \( S_\bot
= S \cup \{ \bot \} \) and \( \bot \notin S\).\symbo{states}\symbo{diver} The
variation of high input is an \ndx{equivalence relation} \(=_L\)\symbo{eql}. Two
inputs are equivalent if they agree on low values, \ie \(s =_L
s'\)\symbo{state0}\symbo{eql}\symbo{state1} iff \( s_l = s_l'
\)\symbo{low}\symbo{state0}. The attacker's\index{attacker (adversary)}
observational power is characterized by a relation on behaviors defined by the
semantics, denoted \(\approx_L\)\symbo{eqax}. Two behaviors are related by
\(\approx_L\)\symbo{eqax} iff they are
indistinguishable\index{indistinguishability} to the attacker\index{attacker
(adversary)}, \ie from the low security class viewpoint of the system. Formally,
program \pr|C|\symbo{p2} is secure \wrt non-interference iff
\begin{align*}
\forall s_1,s_2,\in S \cdot s_1 =_L s_2\implies\sem{C}_{s_1}\approx_L\sem{C}_{s_2}
\end{align*}
\symbo{eqax}\symbo{states}\symbo{state0}\symbo{eql}\symbo{funprog}
which reads:
\begin{quotation}
...
attacker\index{attacker (adversary)}.
\end{quotation}
The security typing rules are shown in~\autoref{fig:ni-types}. For the typing
rules, an expression \(\vdash\prm{e} : \tau\)\symbo{vdash}\symbo{tau} means
\pr|e| has type \(\tau\)\symbo{tau}. The judgement
\(\Gamma\vdash\text{\pr|C|}\)\symbo{G}\symbo{vdash}\symbo{p2} means the program
\pr|C|\symbo{p2} is typable in security context \({\Gamma}\)\symbo{G}.
Expression types and security contexts can be either low (\({l}\)\symbo{low}) or
high (\({h}\)\symbo{high}). The symbol \(\Gamma\)\symbo{G} refers to the current
context (low or high).
...
\centering\drules[E]{
 $\vdash \prm{e} : \tau$
 \symbo{tau}\symbo{vdash}\symbo{low}\symbo{high}
}{expression typing}{High,Low}
\centering\drules[C]{
  $\Gamma \vdash \text{\pr|C|}$
 \symbo{G}\symbo{vdash}\symbo{p2}\symbo{low}\symbo{high}
}{command typing}{Skip,AssignOne,AssignTwo,Seq,While,If,Sub}
...
the programs are written in \ndx{Java}, the constructs are richer than available
in the type system \ndx{grammar}. The programs are not fully analyzable. For
example, the \ndx{security type system} does not specify how to handle the array
...
\ndx{grammar}. The following examples show basic \ndx{non-interference}
derivations.

\begin{example}[Secure high conditional incremental \ndx{leak}]
...
will analyze both cases. We map the program to the \ndx{grammar} by substituting
...
Using a similar procedure as in~\autoref{ex:high-cond-sec}, we express the program in the type system \ndx{grammar} as
\begin{center}
...
The paper then shows that terminating and well-typed programs are computable in polynomial time\ccxi{p}.
...
{object-oriented (OO) programs\index{object oriented SAFE programs}}~\cite{hainry2015}.
...
The term feasible refers to computable in polynomial time~\cite{feree2018}\ccxi{p}.
...
The formalization of \ndx{safe recursion} was motivated by its envisioned
usefulness in cryptographic proofs\index{cryptography}~\cite{heraud2011}. At the
time of its appearance, the technique was integrated into the
\href{https://github.com/EasyCrypt/certicrypt} {\ndx{Certicrypt}} proof
assistant, for cryptographic proofs~\cite{barthe2009}. However, Certicrypt has
since then been superseded by \ndx{EasyCrypt}~\cite{barthe2014}. The support for
...
The \ndx{type system} \ndx{d\(\ell\)T}~\cite{baillot2015,baillot2019} is
simultaneously complexity-aware and has capabilities to support cryptographic
proofs\index{cryptography}. Among its applications, d\(\ell\)T allows proving
that the constructed adversary for the \ndx{Goldreich-Levin Theorem} is
polynomial time\ccxi{p}. Such applications are interesting because they involve
...
no longer guarantees \ndx{complexity bound}s, it demonstrates the benefit of
connecting techniques between the ICC and security.

Adjusting the logic to track \ndx{non-interference} revealed various
mathematical insights. For example, the treatment of loops is more
straightforward for \ndx{non-interference} than \ndx{complexity analysis},
because the latter requires fixed point computations. As a \ndx{data flow
analysis}, that tracks different program properties based on
\emph{dependence}\index{data dependence}, the QI framework is reminiscent of the
\ndx{Dependency Core Calculus} (DCC)~\cite{abadi1999b}. The multiplicity of the
