...
The gold standard in formal verification\index{formal verification} is showing
that an implementation of a program is {totally correct}\index{total
correctness}. Total correctness requires proving two conditions about the
program: that the program meets its specification\index{specifications}, and
...
theory\index{complexity theory} because it uses programming languages as the
...
classes}\index{complexity classes}. The classes describe resource requirements
...
For example, it facilitates formal verification\index{formal verification} of
programs. ICC enables defining {syntactical}
specifications\index{specifications} of resource consumption, and developing
...
lambda-calculus\index{lambda-calculus}, function algebras\index{function
algebras}, quantum\index{quantum computing}, term rewrite systems\index{term
rewrite system}, \(\pi\)-calculus\index{pi-calculus}, \ldots),
restriction-techniques (linear logic\index{linear logic}, data-flow
analysis\index{data flow analysis}, type systems\index{type system}, category
theory\index{category theory}, \ldots), and complexity classes (\textsc{p},
...
Besides uses in formal verification\index{formal verification}, there are many
...
classes\index{complexity classes} and yields more natural definitions and proofs
of central results than classical complexity
theory~\cite{kristiansen2017}\index{complexity theory}. For example, ICC systems
...
soundness~\cite{keidel2021}\index{soundness}, \ie that the analysis technique
...
data size during computation. For imperative programs\index{imperative
programs}, it aims to determine if it is possible to bound the growth of
...
{other} non-functional program properties\index{non-functional property}. The
...
programs\index{imperative programs}. However, the QI framework supports a richer
...
reduced\index{loop transformation}.
...
potential?\index{parallelization potential}
...
nondeterminism\index{nondeterminism}, developing an efficient and useful
...
\IfFileExists{icc}{\subsubsection*}{\paragraph*}{On automatic resource
analysis.} We can now confirm that it is possible to obtain automatic program
...
specification inference\index{specifications}. There are potentially many more
...
(DCC)~\cite{abadi1999b}\index{Dependency Core Calculus}. In DCC, different
...
analysis\index{static program analysis}---and continue the exploration of its
applied potential.
