...
\begin{figure}[h]
\centering
\includegraphics[width=.85\textwidth,keepaspectratio]{fig_bugv}
...
\item \href{https://costa.fdi.ucm.es/~costa/pubs/pubs.php}{\emph{PUBS}: Practical Upper Bounds Solver}\index{PUBS}~\cite{albert2010}
      computes closed form upper bounds for cost equation systems\index{cost equation system};
      specifically, for the \pr|entry| and all cost equations that depend on \pr|entry|.

\item \href{https://aprove.informatik.rwth-aachen.de}{\emph{AProVE}}\index{AProVE}~\cite{giesl2017}
      produces termination proofs and complexity bounds for
      term rewrite systems (TRS)\index{term rewrite system},
      Java\index{programming language!Java} bytecode\index{bytecode},
      and programs written in
      C\index{programming language!C},
      Prolog\index{programming language!Prolog}, or
      Haskell\index{programming language!Haskell}.
...
\item \href{https://github.com/aeflores/CoFloCo}{\emph{CoFloCo}}\index{CoFloCo}~\cite{floresmontoya2014}
...
\item \href{https://www.cs.yale.edu/homes/qcar/aaa/}{\emph{C4B}\index{C4B}:
      ...
      by amortization\index{asymptotic tight bound} -- \autoref{tab:atva-compare}.

\item  \href{http://cl-informatik.uibk.ac.at/software/tct/}{\emph{TcT}\index{TcT}: Tyrolean Complexity Tool}~\cite{avanzini2016}
...
\item  \href{https://koat.verify.rwth-aachen.de/cfr_mprf}{\emph{KoAT}\index{KoAT}:
...
\item   \href{https://www.raml.co/about/}{\emph{RaML}\index{RaML}: Resource aware ML}~\cite{hoffmann2017}
       is a functional programming language, and an analuyzer to compute upper bounds of cost for programs
       written in RaML or OCaml\index{programming language!OCaml}.
...
\item \href{http://cl-informatik.uibk.ac.at/users/zini/software/hosa/}{\emph{HoSA}\index{HoSA}:
...
\item \href{https://github.com/davidnowak/cecoa}{\emph{Cecoa}}~\cite{feree2018}
       ...
       It assists in producing proofs that show a program is poly-time computable\ccxi{p} -- \autoref{icc-formally}.

\item \href{https://costa.fdi.ucm.es/maxcore/}{\emph{MaxCore}\index{MaxCore}}~\cite{albert2019}
...
\item \href{https://www-sop.inria.fr/members/Martin.Avanzini/software/eco-imp/}%
      {\emph{ecoimp}\index{eco-imp}}~\cite{avanzini2020}
      computes expected cost bounds for programs written in a probabilistic imperative \textsc{pWhile} language.

\item \href{https://gitlab.inria.fr/complexityparser/complexityparser/-/tree/master}{\emph{ComplexityParser}\index{ComplexityParser}}~\cite{hainry2021}
      is a complexity analyzer for Java\index{Java}\index{programming language!Java} programs.
...
\item \href{https://github.com/statycc/pymwp}{\emph{pymwp}\index{pymwp}}~\cite{aubert2023b}
      is an implementation of the flow calculus of mwp-bounds.
      It analyzes C\index{C}\index{programming language!C} programs to determine if the program's variable value growth is polynomially bounded in inputs -- \autoref{sec:atva}.
