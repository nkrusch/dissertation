...
on resource requirements, is the natural dual to the study of \ndx{complexity
class}es. Since implicit computational complexity focuses on language-based
...
sufficient expressive power\index{expressiveness} and an efficient evaluation
...
analysis\index{resource analysis} is a subfield of \ndx{static program
analysis}.
...
\item Termination\index{termination} analyses tend to focus on values that are
    ...
    \ndx{termination}. A specific technique for \ndx{termination} analysis, that
    originates from implicit computational complexity, is the \ndx{size-change
    termination principle} of~\textcite{lee2001}.
...
\item[\href{https://koat.verify.rwth-aachen.de/cfr_mprf}{KoAT}]
       ...
       program} and C integers programs that follow
       \href{https://termination-portal.org/wiki/C_Integer_Programs}{the
       \ndx{grammar} specification} of the Termination Competition --
...
\item[\href{https://www.raml.co/about}{RaML}]
       ...
       cost, for functional programs written in OCaml\index{OCaml}. The cost
       resource can be any quantity that is of interest. The RaML analysis is
       based on AARA and can produce precise amortized\index{amortization}
       bounds.
...
In characterizing a \ndx{complexity class}, a \emph{\ndx{soundness}} statement
says that all programs of a given language, or those satisfying a restriction
\(\mathcal{R}\)\symbo{restriction}, admit a certain complexity property, \ie
compute functions in the target class. A \emph{\ndx{completeness}} statement
says that all functions of the corresponding functional complexity class can be
programmed in this language~\cite{baillot2012}. Alternatively, if
\(\mathcal{R}\)\symbo{restriction} is \emph{(extensionally)
complete}\index{extensional completeness}, then for any problem in the
\ndx{complexity class}, there exists a program in
...
\ndx{complexity class}, but the program fails the analysis because it is not
expressible\index{complexity analysis} in the ICC system. Fixing the issue
requires rewriting the program to satisfy the class membership restriction
\(\mathcal{R}\)\symbo{restriction}. Due to \ndx{undecidability}, it is
impossible to completely avoid false negatives. Stated differently, implicit
...
\enquote{completely correct} about membership in a particular \ndx{complexity
class}. It requires defining a non-Turing-complete programming language and
proving that it is sound\index{soundness} and complete\index{completeness} \wrt
a particular \ndx{complexity class}. This sidesteps \ndx{Rice's Theorem}, but
cannot be \emph{intentionally} complete, since it will always exclude programs
that yet belong to the targeted \ndx{complexity class}. Correctly analysing a
real world programming language is very difficult because of the large number of
constructions allowed in it.
