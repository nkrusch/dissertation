...
on resource requirements\index{resource usage}, is the natural dual to the study
of \ndx{complexity classes}. Since implicit computational complexity focuses on
...
systems---with sufficient expressive power\index{expressiveness} and an
efficient evaluation strategy---enable concrete program analysis. An analysis
that evaluates a program's \ndx{resource usage} is called resource analysis.
Resource analysis\index{resource analysis} is a subfield of \ndx{static program
analysis}.
...
the \ndx{precision} and scalability of the techniques. These topics are actively
...
sacrifice something among \ndx{soundness}, \ndx{completeness} and
\ndx{termination}~\cite{moller2024}. Although seemingly discouraging, \ndx{Rice's
...
by testing\index{software testing}. The analysis must be precise because
excessive false alarms make the tool unusable. The \emph{\ndx{precision}} of an
...
Obtaining the intended behavior depends on design choices, particularly on how
...
Program analysis as an approximation -- an illustration inspired
by~\textcite{steffen2020}. The \textsc{exact} class of programs satisfying the
analyzed property \(\phi\)\symbo{phi} is undecidable.\index{undecidability} The
program \(a\) satisfies \(\phi\)\symbo{phi}, but the program \(b\) does not. For
an analyzer making a \textsc{safe under-approximation}, every accepted program
truly satisfies \(\phi\), but would give an erroneous result on \(a\). An
analyzer making a \textsc{safe over-approximation} would accept \(a\) and \(b\),
even though \(b\) does not satisfy \(\phi\). However, an over-approximating
analyzer never misses programs that satisfy \(\phi\). Although an
\textsc{unsafe} analysis can be justifiable in some cases, interpreting its
results is problematic because errors are not conservative.
}\label{fig:approx}
\end{figure}

\paragraph*{Soundness and \ndx{completeness}}\index{soundness}
...
program analysis}. In logic, they carry the following meaning.
\begin{center}
\begin{minipage}{\textwidth}
\begin{itemize}
\item A \emph{sound}\index{soundness}
system guarantees that everything that is provable is true.
\item A \emph{complete}\index{completeness}
system guarantees that everything that is true has a proof.
\end{itemize}
\end{minipage}
\end{center}
In the context of program analysis, if a provably sound system reports that some
property holds, then it must be true for the analyzed program. A
sound\index{soundness} analyzer never misses a violation---\ie when a property
does \underline{not} hold---but it may report an erroneous result for a
satisfactory program~\cite{torlak2015}. A complete\index{completeness} analyzer
detects all satisfactory programs, but also accepts cases where the tracked
property does not truly hold. The analysis approximation should be
\emph{conservative}\index{conservative analysis} whereby all errors occur on the
\enquote{safe side} of the application domain~\cite[p. 5]{moller2023}. Refer to
Figures~\ref{fig:approx} and~\ref{fig:bug-verify} for examples.
...
Soundness\index{soundness} is challenging because it can render the analysis
unscalable or imprecise to the point of being useless. To maintain semantics, an
optimizing analysis typically requires \ndx{soundness}~\cite[p. 5]{moller2023}.
A complete\index{completeness} analyzer underestimates the program behaviors and
thus errs on the other side.

\paragraph*{Soundiness}\index{soundy}
...
insist on \ndx{Turing-completeness} and \ndx{termination}, but trade between
\ndx{soundness} and \ndx{completeness}~\cite{moller2023,steffen2020}. As an
compromise, modern analyzers for real programming languages are often
\ndx{soundy}. A \emph{soundy} analysis is as sound as possible, without
excessively compromising \ndx{precision} or scalability~\cite{livshits2015}.

\subsubsection{Techniques of Resource Analysis}
\label{resource-analysis}

Resource analysis\index{resource analysis}\footnote{Also called \emph{cost
analysis} when using an explicit \emph{cost measure}\index{complexity
measure}~\cite{albert2008}. }---or (static/automatic) {complexity
...
time\index{time complexity} and \ndx{termination}; and the other change in the
program's data size~\cite{jones2009}\index{space complexity}. Numerous
...
\item Termination analyses tend to focus on values that are copied or decreased.
    Establishing \ndx{termination} is a critical aspect of correctness of
    ...
    sequence is finite is called the \emph{\ndx{strong normalization}}
    property~\cite[p. 36]{bertot2004}}--- that guarantee \ndx{termination}. A
    specific technique for \ndx{termination} analysis, that originates from
    implicit computational complexity, is the \ndx{size-change termination
    principle} of~\textcite{lee2001}.
...
analysis\index{amortized analysis} (AARA) and \ndx{cost equation system}s (CES).
These techniques are briefly introduced next.

\paragraph*{Automatic amortized resource analysis}\index{automatic amortized resource analysis\dotfill}
\emph{Amortized analysis}\index{amortized analysis} averages the time required
to perform a sequence of operations over all the operations performed~\cite[p.
451]{cormen2009}. Amortization\index{amortization} enables to show that the
average cost of an operation is small, even if a single operation might be
expensive.

\emph{Automatic amortized resource analysis}\index{automatic amortized resource analysis\dotfill}
(AARA)~\cite{hoffmann2022}\index{amortized analysis} builds
on the same idea: bounding programs by \ndx{amortization}. In AARA, the analysis
can account for \ndx{amortization} effects across sequences of operations.
Internally, the analysis relies on \ndx{inference rule}s and \ndx{derivation
tree}s provide proofs of the \ndx{resource bounds}. Thus, AARA provides an
...
programs}~\cite{carbonneaux2015,carbonneaux2017}, parallel~\cite{hoffmann2015},
probabilistic\index{probabilistic
...
\item[\href{https://aprove.informatik.rwth-aachen.de}{\ndx{AProVE}}]
      ...
      \ndx{complexity bound}s for \ndx{term rewrite system}s (TRS), \ndx{Java
      bytecode}\index{bytecode}, and programs written in \ndx{C}, \ndx{Prolog},
      or \ndx{Haskell}.
      \ndx{AProVE} is a regular contestant in international tool competitions,
      including the
...
\item[\href{https://github.com/aeflores/CoFloCo}{\ndx{CoFloCo}}]
    ...
    equation system solver, that infers symbolic upper\index{upper bound} and
    lowe\index{lower bound} \ndx{complexity bound}s for
...
\item[\href{https://github.com/academic-archive/pldi15}{C4B}]
      ~\cite{carbonneaux2015}\index{C4B}
      is an AARA-based analyzer of certified compositional resource bounds. It
      infers linear resource bounds on \ndx{C}-like programs by \ndx{amortization} --
...
\item[\href{http://cl-informatik.uibk.ac.at/software/tct/}{TcT}]
       ...
       ~\cite{avanzini2016}\index{TcT}
       is a modular analyzer with front-ends to support TRSs and integer
       transition systems (ITS); high-order functional programs, and \ndx{Java
       bytecode}. The complexity problem and metric of interest are parametric.
       TcT has also competed in the \ndx{Termination Competition}.

\item[\href{https://koat.verify.rwth-aachen.de/cfr_mprf}{KoAT}]
       \emph{\underline{Ko}mplexitäts-\underline{A}nalyse-\underline{T}ool}
       ~\cite{brockschmidt2016}\index{KoAT}
       is a complexity analyzer for computing time-, cost- and \ndx{size
       bound}s. As inputs, KoAT accepts Integer Programs (IPs) and \ndx{C}
       \ndx{integer program}s that follow
       \href{https://termination-portal.org/wiki/C_Integer_Programs}{the
       \ndx{grammar} specification} of the \ndx{Termination Competition} --
...
\item[\href{https://github.com/statycc/LQICM_On_C_Toy_Parser}{LQICM}]
      ...
      \underline{C}hunk \underline{M}otion}~\cite{moyen20172}\index{LQICM} is a
      compile-time program optimization pass for \ndx{C} programs. It aims to
...
\item[\href{https://www.raml.co/about}{RaML}]
       ...
       ~\cite{hoffmann2017}\index{RaML}
       is an analyzer of worst-case upper bounds and best-case \ndx{lower
       bound}s of cost, for functional programs written in \ndx{OCaml}. The cost
       resource can be any quantity that is of interest. The \ndx{RaML} analysis
       is based on AARA and can produce precise amortized\index{amortization}
       bounds.
...
\item[\href{https://github.com/academic-archive/cav17-pastis}{\ndx{Pastis}}]
      ...
      change information of programs expressed in \ndx{LLVM bitcode} (interprocedural
...
\item[\href{http://cl-informatik.uibk.ac.at/users/zini/software/hosa/}{HoSA}]
      ...
      \underline{A}nalysis}~\cite{avanzini2017}\index{HoSA}
...
\item[\href{https://github.com/davidnowak/cecoa}
     {Cecoa}]\cite{feree2018}\index{Cecoa}
     ...
     computable\ccxi{p} -- \autoref{subsubsec:icc-formally}.
...
\item[\href{https://costa.fdi.ucm.es/maxcore/}{MaxCore}]%
      ...
      solver}~\cite{albert2019}\index{MaxCore} is an upper bounds complexity
      analyzer for C programs\index{C}. The underlying technique is based on
...
\item[\href{https://www-sop.inria.fr/members/Martin.Avanzini/software/eco-imp/}
      {eco-imp}]\cite{avanzini2020}\index{eco-imp}
...
\item[\href{https://gitlab.inria.fr/complexityparser/complexityparser}
      {ComplexityParser}]\cite{hainry2021}\index{ComplexityParser}
      is a complexity analyzer for Java\index{Java} programs. A typable program
...
\item[\href{https://github.com/statycc/pymwp}{pymwp}]%
      \cite{aubert2023b}\index{pymwp}
      is an implementation of the flow calculus of \ndx{mwp-bound}s. It is
      purely syntactical analysis of C\index{C} programs, to determine if the
...
In characterizing a complexity class\index{complexity classes}, a
\emph{\ndx{soundness}} statement says that all programs of a given language, or
those satisfying a restriction \(\mathcal{R}\)\symbo{restriction}, admit a
certain complexity property, \ie compute functions in the target class. A
\emph{\ndx{completeness}} statement says that all functions of the corresponding
functional complexity class can be programmed in this
language~\cite{baillot2012}. Alternatively, if
\(\mathcal{R}\)\symbo{restriction} is \emph{(extensionally)
complete}\index{extensional completeness}, then for any problem in the
complexity class\index{complexity classes}, there exists a program in
\(\mathcal{R}\)\symbo{restriction} that decides it. For instance, for polynomial
time\ccxi{p} complexity, the \ndx{soundness} statement refers to a polynomial
time evaluation of programs, whereas \ndx{completeness} refers to the class FP
of functions computable in polynomial time~\cite{baillot2012}. However,
\ndx{extensional completeness} is of limited interest, as the goal is to
validate as many interesting programs as possible.

ICC systems are typically proven sound \wrt the complexity class they
...
Complexity analysis\index{complexity analysis} can then give an erroneous result
in the following way: a developer writes a program that belongs to the intended
complexity class\index{complexity classes}, but the program fails the analysis
because it is not expressible\index{complexity analysis} in the ICC system.
Fixing the issue requires rewriting the program to satisfy the class membership
restriction \(\mathcal{R}\)\symbo{restriction}. Due to \ndx{undecidability}, it
...
\paragraph*{Input languages and expressive power}\index{expressiveness}
Extensional correspondence\index{extensional completeness} with complexity
...
\ndx{Turing-completeness} allows developing an analysis that is
\enquote{completely correct} about membership in a particular complexity
class\index{complexity classes}. It requires defining a non-Turing-complete
programming language and proving that it is sound\index{soundness} and
complete\index{completeness} \wrt a particular complexity
class.\index{complexity classes} This sidesteps \ndx{Rice's Theorem}, but cannot
be \emph{intentionally} complete\index{completeness}, since it will always
exclude programs that yet belong to the targeted complexity
class.\index{complexity classes} Correctly analysing a real world programming
...
Turing-complete\index{Turing-completeness} language for example the class of
PTime\ccxi{p} programs is not recursively enumerable, and therefore an
intentionally complete criterion would not be decidable~\cite[Sect.
4.1]{mogbil2012}.\index{undecidability}
\end{quotation}

\noindent For this reason, most research\footnote{
Among exceptions, \ndx{quasi-interpretation}s~\cite{marion2000} considers a
Turing-complete\index{Turing-completeness} language.} in implicit complexity
techniques focuses on \enquote{toy languages}~\cite{moyen2017,rubiano17}; like
\ndx{term rewrite system}s or the \textsc{Loop} language~\cite{kristiansen2005}.
...
That ICC systems are designed to characterize a specific \ndx{complexity
classes} has implications on the results they produce. If they generate bounds,
...
improving \ndx{precision} of obtained \ndx{resource bounds}~\cite{benamram2020}.
However, if the system is designed as a sound\index{soundness}
under-approximation, it can only provide \ndx{upper bound}s~\cite[p.
...
class~\cite{baillot2019}. On the other hand, unlike many other \ndx{static
program analysis} techniques, ICC systems may not require establishing
\ndx{termination} and avoid difficult control-flow analysis by abstraction and
...
The second strategy allows a software engineer to write any program and performs
program analysis \emph{aposteriori}. Instead of requiring every program to be
written using the restriction \(\mathcal{R}\)\symbo{restriction}, we would test
the adhesion of the program to the restriction
\(\mathcal{R}\)\symbo{restriction}. While this is more preferable for the
program creator, such analysis is necessarily incomplete.\index{completeness}
...
Turing-complete\index{Turing-completeness} programming language. The first
...
Turing-complete\index{Turing-completeness} programming languages. Let us not
...
already exist. The safe subset of the programming language \ndx{Rust} guarantees
\eg memory safety and \ndx{deadlock freedom} by design. Strongly normalizing
interactive theorem provers require \emph{structural} fixed point
\ndx{termination}~\cite{bertot2004}. In return, every valid programs is known to
...
\emph{\ndx{X-by-Construction}}~\cite{terbeek2018} guarantees.
