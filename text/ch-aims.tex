...
% These amazing advances in computer application can distract attention from
%the fact that Computer Science also has a central core of fundamental discoveries
%which are particular to itself as an independent intellectual discipline. Computing Research is driven, like research in other mature branches of pure science, by natural curiosity, exploring the basic foundations and limitations of the programmable computer, independent of any particular area of application. Because of its effective combination of pure knowledge and applied invention, Computer
%Science can reasonably be classified as a branch of Engineering Science. \cite{hoare2006}
...
A supporting case exists in automata theory\footnote{
The classic formulation of Parikh's Theorem concerns language expressiveness.
Its usefulness in applications was enabled {only decades later}, by the development of efficient algorithms,
enabling the exploitation of Parikh's Theorem in applications including SMT-solvers, verification of cryptographic protocols and concurrent programs, query evaluation over graph databases, \etc~\cite[pg. 2]{hague2024}}.
...
The listings will display, in bottom right corner, the associated programming language or context according to~\autoref{tbl:pls}.

\begin{table}[h]
\begin{center}
\begin{tabular}{@{}lcl@{}}
\toprule
Language & Color & Description \\
\midrule
C    && the C language \\
C*   && a generic C-\emph{like} language (more broadly). \\
omp  && C code that includes OpenMP directives \\
imp  && the simple imperative language of mwp-calculus \\
cmd  && runnable terminal (console) command \\
\bottomrule
\end{tabular}\end{center}
\caption{Code listings languages.}
\label{tbl:pls}
\end{table}
...
This is the convention of the \emph{Z specification language}\index{Z specification language}~\cite[p. 34]{furia2014} of~\textcite{spivey1992}.
