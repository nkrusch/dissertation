...
is a parallel \ndx{programming model} for developing portable, \ndx{shared memory} parallel applications in
\pr|C|,\index{C}
\pr|C++|\index{C++} and
\pr|Fortran|\index{Fortran}~\cite{openmp_api}.
OpenMP is not a programming language.
Rrather, it provides directives\index{OpenMP directives} for sequential programs that translate the programs to parallel execution instructions at compile-time.
...
\index{fork-join programming model}\index{parallel programming model!fork-join} of~\autoref{fig:fork-join}.
...
Such non-canonical loops are traditionally outside the built-in transformation and parallelization capabilities of \ndx{parallel programming model}s like OpenMP\@.
