...
This section focuses on automatic compile-time loop \emph{transformations}\index{loop transformations} and relating those transformations to \emph{parallel executions}.
...
In~\autoref{lst:loop-ex}, the header of the nested loop is at L\ref{line:header-2} and body\index{loop body} at L\ref{line:body}.
...
The definition is specific to the programming model of~\autoref{openmp}\index{parallel programming models!OpenMP} and~\autoref{sec:vmcai}.} has form,
\begin{center}
\begin{minipage}{\textwidth}
\captionsetup{type=lstlisting}
...
are a common approach for compile-time program analyses and transformations\index{loop transformations}~\cite[p. 48]{moyen2017}
...
Loop \emph{fusion}\index{loop transformations!fusion}, in~\autoref{lst:fusion}, merges multiple loops by replacing them with a single loop.
In each iteration, the generated loop executes an iteration of each original loop.
Loop \emph{fission}\index{loop transformations!fission}, in~\autoref{lst:fission}, operates in the reverse direction.
...
\paragraph*{Other loop transformations}
Although the main focus of the dissertation work is loop fission (in~\autoref{sec:vmcai}), there are several other loop transformations.
The transformations are summarized visually in~\autoref{fig:loop-transforms}.
When considering the overall compilation strategy, the combination of applied transformations is important.
...
\begin{center}
    \begin{minipage}{\textwidth}
        ...
        \begin{minipage}[t]{.45\textwidth}
            ...
            \captionof{lstlisting}[Before interchange]{Pre-interchange.}
            \label{lst:pre-interchange}
        \end{minipage}\hfill%
        \begin{minipage}[t]{.45\textwidth}
            \cstarinputlisting{post-interchange.c}
            \captionof{lstlisting}[After interchange]{Post-interchange.}
            \label{lst:post-interchange}
        \end{minipage}
    \end{minipage}
\end{center}

\captionof{figure}[Visualizing loop transformations]
{Visualizing loop transformations.}
...
\item[\itemindent=0pt Iteration range-based transformations.]\mbox{}
\begin{itemize}[leftmargin=*]
\item The \emph{split} transformation\index{loop transformations!split},
in~\autoref{lst:split}, breaks the original loop into multiple generated loops.
Each generated loop has the same body as the original, but iterates over a
different contiguous subset of the original range.

\item Loop \emph{tiling}\index{loop transformations!tile} breaks a loop into contiguous
chunks of fixed \emph{tile size}. In~\autoref{lst:tiling}, the tile size is 8.
Tiling improves cache behavior by reducing data reuse
distance~\cite{bertolacci2018}.

\item Loop \emph{reversal}\index{loop transformations!reversal} executes loop
iterations in the reverse order. For example, if \(0, 1,\ldots, n-2, n-1\) are
the logical iteration numbers of a loop, after transformation, the iterations
occur in order \(n-1, n-2,\ldots, 1, 0\)~\cite{openmp_api}.

\item Finally, loop \emph{peeling}\index{loop transformations!peel}, in~\autoref{lst:peel}, is a special case of loop splitting\index{loop transformations!split}.
...
\item[Modifications of the loop body.]
Loop \emph{unrolling}\index{loop transformations!unroll} expands and duplicates the body commands into similar independent commands.
...
\item[Transforming nested iterations.]
Loop \emph{interchange}\index{loop transformations!interchange} is a transformation on a nested loop.
...
\label{dep-anti}\index{antidependence}\index{dependence!anti}
\end{minipage}
...
\label{dep-out}\index{output dependence}\index{dependence!output}
\end{minipage}\hspace{3em}
\begin{minipage}[t]{.4\textwidth}
\captionsetup{type=lstlisting}
...
\label{dep-control}\index{control dependence}\index{dependence!control}
\end{minipage}
\end{center}
...
\item An \emph{\ndx{antidependence}}\index{dependence!anti}
reverses the order,
...
{\centering\includegraphics{fig_loop-carry}}
\end{minipage}
...
Dependence analysis\index{dependence analysis} (or a \emph{dependence test}) is
a technique for identifying or characterizing program dependencies and
dependence-preserving reorderings; which may then permit transformation or
parallel execution. Among the well-established techniques of \ndx{dependence
analysis} are \ndx{Banerjee test}~\cite{banerjee1997}, \ndx{GCD
test}~\cite{zima199}, \ndx{Omega test}~\cite{pugh1991}, and \ndx{commutativity
analysis}~\cite{rinard1996}\index{commutativity analysis}. Section
\autoref{sec:vmcai} will present a complementary \ndx{dependence analysis}
inspired by implicit computational complexity.
...
\paragraph*{Parallel programming models}\index{parallel programming models}
...
The OpenMP\index{OpenMP}\index{parallel programming models!OpenMP}
application programming interface is presented in~\autoref{openmp}.
Other approaches include the
\emph{Parallel Patterns Library} (PPL)\index{PPL}\index{parallel programming models!PPL}~\cite{ppl2021,campbell2011},
that parallelizes tasks through built-in algorithms and containerization;
and the \emph{oneAPI Threading Building Blocks} (oneTBB)\index{oneTBB}\index{parallel programming models!oneTBB},
a runtime-based model for \pr|C++|\index{C++} programs~\cite{onetbb}.

\subsubsection{The OpenMP Programming Model}
\label{openmp}

{OpenMP}\index{OpenMP}\index{parallel programming models!OpenMP}, for \emph{Open Multi-Processing},
is a parallel programming model for developing portable, \ndx{shared memory} parallel applications\index{concurrent programs} in
\pr|C|,\index{C}
\pr|C++|\index{C++} and
\pr|Fortran|\index{Fortran}~\cite{openmp_api}.
OpenMP is not a programming language.
Rather, it provides \emph{directives}\index{OpenMP directives} for sequential programs that translate the programs to parallel execution instructions at compile-time.
...
Program execution in OpenMP follows the \emph{fork-\ndx{join} \ndx{programming model}}~\cite[p. 24]{chapman2007}
\index{fork-join programming model}\index{parallel programming models!fork-join} of~\autoref{fig:fork-join}.
...
After encountering a specially-marked parallelizable program region, the primary thread spawns a team of threads in a \emph{\ndx{fork}}-operation.
The team collaborates to complete the tasks of the parallel program region.
The execution concludes with a \emph{\ndx{join}}-operation, where the forked threads terminate.
...
\caption[The fork-join programming model]{The fork-\ndx{join} programming model.}
\label{fig:fork-join}
\end{figure}

In OpenMP, a programmer annotates {structured blocks} with parallelization {directives}\index{OpenMP directives} to control the parallel execution behavior.
...
Because OpenMP is a powerful and rich programming model\index{parallel programming models}, it is possible to present only a light introduction.
...
\end{minipage}\hfill%
\begin{minipage}{.5\textwidth}
\hfill\includegraphics{fig_openmp}
\end{minipage}
...
It prevents any thread\index{thread} from continuing past the barrier until all threads of the same team encounter the barrier.
...
\end{minipage}\hfill%
\begin{minipage}{.45\textwidth}
\hfill\includegraphics{fig_exec-seq}
\end{minipage}
...
\end{minipage}\hfill%
\begin{minipage}{.45\textwidth}
\hfill\includegraphics{fig_exec-parallel}
\end{minipage}
...
\end{minipage}\hfill
\begin{minipage}{.45\textwidth}
\hfill\includegraphics{fig_exec-for}
\end{minipage}
...
OpenMP includes built-in directives that perform loop transformations\index{loop transformations}~\cite{openmp_api}.
...
fusion\index{loop transformations!fusion},
interchange\index{loop transformations!interchange},
reverse\index{loop transformations!reversal},
split\index{loop transformations!split},
stripe\index{loop transformations!stripe} (interleaving of loop iterations),
tile\index{loop transformations!tile}, and
unroll\index{loop transformations!unroll}.
As an example, we consider the unrolling transformation.

\paragraph*{Loop unrolling}\index{loop transformations!unroll}
...
Then, the generated tile\index{loop transformations!tile} loop is fully unrolled.
...
Applying loop transformations\index{loop transformations} requires suitable loop form.
...
Through a loop fission\index{loop transformations!fission} transformation,
...
\begin{figure}[h]
{\vspace{1em}\centering\includegraphics[width=\textwidth,keepaspectratio]{fig_parallel}}
\caption[Loop fission transformation]
{Loop fission of a sequential loop into parallelizable loops.
The loop header (the leading \langclr{header} block) is replicated across the generated parallellizable loops.
In the sequential \ndx{loop body}, the colors denote (disjoint) dependence between command blocks.
Each generated parallellizable loop takes a part of the original loop's body.}\label{fig:icc-fission}
\end{figure}

The technique is interesting because it is applicable even when the loop
iteration space is unknown; enabling parallelization of \eg \pr|while| loops.
Such \ndx{non-canonical loop}s are traditionally outside the built-in
transformation and parallelization capabilities of \ndx{parallel programming
models} like OpenMP\index{parallel programming models!OpenMP}. Although the
\emph{iterations} of \pr|while| loops must be executed under the \omp{single}
construct, the \emph{multiple loops} generated by the transformation can be
parallelized with \ndx{OpenMP directives}. The technique is useful as a
preprocessor, before adding parallelization directives; and in conjunction with
other \ndx{loop transformations}. The technique is inspired
by~\cite{moyen20172}, but was adjusted significantly to support the different
application.
