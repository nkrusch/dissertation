...
\pr|\ndx{C}|,
\pr|\ndx{C++}| and
\pr|\ndx{Fortran}|~\cite{openmp_api}.
OpenMP is not a programming language;
rather, it provides directives\index{OpenMP directives} for sequential programs that translate the programs to parallel execution instructions at compile-time.
...
Program execution in OpenMP follows the \emph{fork-\ndx{join} programming model}~\cite[p. 24]{chapman2007}\index{fork-join programming model} of~\autoref{fig:fork-join}.
