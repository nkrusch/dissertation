...
derivation choices through \emph{\ndx{polynomial structure}s}, and introducing
...
The developments reveal deeper insights of the flow calculus\index{mwp-calculus}.
...
distributed\index{loop transformations!distribution} between the available
...
\ndx{automatic resource analysis} and ICC, and the challenges that emerge from
...
targeted complexity class\index{complexity classes}. Like in \ndx{safe
recursion} (\autoref{safe-rec}), no explicit bound needs to be provided~\cite[p.
13]{moyen2017}.
...
{cost equation system}s are different from the analyzers of \ndx{C} programs.
...
alternative. Although some concerted efforts---like the \ndx{Termination
Competition}---push to establish a shared I/O format, there is no widely adopted
standard.
...
calculus\index{mwp-calculus}, we learned through trial-and-error that
...
questions relating to the loop transformation\index{loop transformations}
...
We must consider combinations of loop transformations---like fusion\index{loop
transformation!fusion}, tiling\index{loop transformations!tiling}, and
interchange\index{loop transformations!interchange}, \etc---and their combined
