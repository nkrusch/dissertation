\subsection*{Abstract}
Non-interference is an information flow policy for guaranteeing confidentiality,
...
studied by attaching security classes to variables, then analyzing the classes
to determine if a violation, or a data \enquote{leak}, can occur. Security type
systems are common controls for analyzing and enforcing non-interference.
...
non-interference}. By anytime, we mean a malicious actor with low-level access
...
\emph{Non-interference}~\cite{goguen1982} is a classical semantic security
...
Assignment]\label{def:ifp} An \emph{information flow policy} is a lattice \(\SC
...
How a security-flow matrix is constructed by induction over the command is
...
\begin{minipage}{4cm}
\lstinputlisting[mathescape,frame=none,numbers=none,aboveskip=0em,belowskip=0em]{if-else.while}
\end{minipage}
\end{lrbox}
...
\begin{lrbox}{\whilec}
\begin{minipage}{4cm}
\lstinputlisting[mathescape,frame=none,numbers=none,aboveskip=0em,belowskip=0em]{arrays.while}
\end{minipage}
\end{lrbox}
