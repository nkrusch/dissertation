...
Assignment]\label{def:ifp} An information flow policy\index{information
flow!policy} is a \ndx{lattice} \(\SC = (\SCset, <)\)\symbo{seclat} where
\(\SCset\)\symbo{secset} is a partially \(<\)-ordered finite set of
...
loops~\cite{aubert20232}.\index{loop transformations!distribution} We assume
...
matrices\index{security-flow matrix} for input programs. The security flow
...
Anytime \ndx{non-interference}\index{non-interference!anytime} is
...
\ndx{termination}-insensitivity is too weak to guard against untrusted code, \eg
...
not enforce \ndx{complexity bound}s, it reinforces the bidirectional connection
between ICC and \ndx{language-based security}.
