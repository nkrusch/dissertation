...
\ndx{invariant}s, \ie conditions that must hold pre-iteration and be preserved
...
\ndx{postcondition}s of the variables occurring in the loop. A postcondition is
...
Given a specification with a \ndx{precondition} (\pr|assume|) and a loop command
(\pr|for|), the goal of formal verification is to prove that the
\ndx{postcondition} (\pr|assert|) is satisfiable. The program \emph{\explain},
in \autoref{lst:exB}, shows the problem variant we address in the paper. When
precise variable values, \ndx{precondition}, and iteration count are unknown,
...
and thus distant from \ndx{numerical loop} analysis.
...
We consider four benchmark suites, of numerical \ndx{C} loops\index{numerical loops}, and 620 total problem (detailed in~\autoref{app:subsec:bench}).
...
The two main directions concern enriching the analysis \ndx{expressiveness} and precision.
...
Discovering ways to relax this restriction would improve \ndx{expressiveness} and yield more postconditions.
