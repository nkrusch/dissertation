...
\(x'_i\)\symbo{xprime2} of \(\prm{X}_i\) (for \(i=1,\ldots,n\)). More precisely,
...
analysis result is derived statically, by applying \ndx{inference rule}s to the
commands of the program, in a compositional\index{compositionality} bottom-up
manner.
...
\emph{derivable}. The flow calculus assigns an \emph{\ndx{mwp-bound}} to every
variable of a derivable program. An \ndx{mwp-bound} is an expression, in terms
...
calculus~\cite[p. 11]{jones2009}\index{soundness of mwp-calculus} guarantees
...
\begin{enumerate*}
    ...
    \item \emph{Precondition}\index{precondition} \(P\), is the initial logic
expressions assumed to hold before entering the loop.

\item Program, \pr|loop b C|, performs command \pr|C| until the expression
\pr|b| becomes false.

\item \emph{Postcondition}\index{postcondition} \(Q\), is the final logic
...
terminates\index{termination}, every call to another procedure satisfies its
\ndx{precondition}s, and the \ndx{postcondition} holds at program
...
An \emph{\ndx{invariant}} describes an \ndx{assertion} about a program location that
...
\ndx{invariant} \(0 \leq \prm{i} \leq \prm{n}\) and a postcondition
...
(\autoref{subsec:imp-language}).\index{imperative programs} We analyze the loop
using the \ndx{inference rule}s of the flow calculus
(\autoref{subsec:matrices})\index{mwp-calculus}. As output, the analysis
produces an \emph{mwp-matrix} (\autoref{subsec:mat-decode})\index{mwp matrix}.
Our enhancements involve {evaluating} these mwp-matrices. We define first an
{evaluation procedure} (\autoref{subsec:eval}) that enables extracting optimal
\ndx{mwp-bound}s from mwp-matrices, and in more cases than previously
(\autoref{subsec:bounds-at-failure}). This provides the \ndx{postcondition}s
that address the problem formulation of~\autoref{subsec:overview}.

\subsubsection{Imperative language of input programs}
\label{subsec:imp-language}

\begin{definition}[Imperative language]
\label{def:lang}\index{imperative program}
...
\begin{align*}
    ...
    \text{\pr|loop X C|} \BNF
    \text{\pr|C;C|}
\end{align*}\symbo{coloneqq}\symbo{bnfsep}
...
coefficients,\index{flow coeﬀicient} how data flows in variables between
...
on.} To trade precision for efficiency, the flow calculus\index{mwp-calculus}
omits evaluating loop iteration bounds and reasoning about \ndx{termination} and
control expressions, overestimating their effect. At conclusion, the flow
calculus assigns \underline{one} \ndx{mwp-matrix} to the analyzed program,
...
expressions.]{
    ...
    {variable} is denoted as \(\prm{X}_i\)\symbo{xvar} where the \(i\) refers to
    an index of a matrix column (\resp \(\prm{X}_j\) for the \(j\)th column). In
    rules E3 and E4, a vector of coefficients is denoted by \(V_n\)\symbo{vec}.
    ...
    of the flow calculus\index{mwp-calculus} comes from these inference rules.
}
\label{fig:rules-expressions}
\end{figure}

Nondeterminism\index{nondeterminism} is necessary to improve the expressive power\index{expressiveness} of the flow calculus, but complicates the analysis procedure.
...
The flow coefficients are elements of the finite set \(\text{\textsc{mwp}}^\infty \triangleq \{0, m, w, p,\infty\}\).\symbo{trieq}
A \emph{domain} \(\mathcal{D} \triangleq \{0, 1, 2 \}\)\symbo{domain} is a bidirectional map of the flow calculus \ndx{inference rule}s.
In other words, the domain members can be translated to flow calculus inference rules (E2--E4) and vice versa.
The \emph{degree} of choice, \(k : \mathbb{N}\)\symbo{k2}, is a counter of derivations.
...
The inferred \ndx{postconditions} can be described by the following categories.
...
definition of optimality\index{mwp-bounds!optimal} does not guarantee the
...
\ndx{assertion}s, two more steps are required. First, we
must record the initial variable values because they are needed to express
\ndx{postcondition}s. Next, we must complete the parts that are left implicit in
\ndx{mwp-bounds}, like constants. \autoref{app:sec:verified} demonstrates how we
perform both steps. In general, the \ndx{postcondition}s provide bounding
...
\ndx{numerical loop} analysis. The \ndx{invariant} detector
...
terminate and its \ndx{guard variable} \pr|X1| does not occur in the body.
In~\ref{lst:case-2}, the loop iteration and \ndx{termination} are unknown since
they are controlled by a nondeterministic function\index{nondeterminism}. The
loop in~\ref{lst:case-3} has a fixed \ndx{iteration space}, but the
\ndx{postcondition} of \pr|x| is difficult to infer. Assuming \(\prm{y}=0\), the
...
precision. \Eg, leveraging \ndx{assumption}s (if available), tracking variable
...
inside \ndx{unbounded loop}s. Discovering ways to relax this restriction would
