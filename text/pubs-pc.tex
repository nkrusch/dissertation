...
\(\vdash\mathcal{C} : M\)\symbo{cmat}. It contains all derivations of the input
...
about a variable at column \(j\)\symbo{mj} are collected in the polynomial
...
\(m\)\symbo{m}{ }coefficients. An \(m\)-flow at the \ndx{mwp-matrix} diagonal
...
characterized by ∞\symbo{infty}. The $\poly_1$\symbo{hp} and  are
\emph{\ndx{honest polynomial}s}, build up from constants and variables by
applying $+$ and $\times$. The variables with \(w\)\symbo{w}-flow occur in
$\poly_1$\symbo{hp} and variables with \(p\)\symbo{p}-flow in
$\poly_2$\symbo{hp}. Any of the three variable lists might be empty, and
$\poly_1$ and $\poly_2$\symbo{hp} may not be present. When characterizing value
growth, an \ndx{mwp-bound} is approximative. It excludes precise constants and
degrees of polynomials. An \ndx{mwp-bound} is a monotonic over-approximation,
\ie it does not decrease if variable value decrease over subtraction. A program
\emph{bound} is a conjunction of its variables' \ndx{mwp-bound}s.
\autoref{ex-bounds} shows illustrative cases of \ndx{mwp-bound}s. The
...
restricted to deriving \ndx{mwp-bounds} for all variables concurrently. For
...
\end{prooftree}
\end{center}
\symbo{matrix}\symbo{cmat}\symbo{xvar}\symbo{closure}
\symbo{p}\symbo{m}\symbo{mi}\symbo{mj}\symbo{mzeroj}
