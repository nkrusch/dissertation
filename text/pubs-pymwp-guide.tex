...
\lstinputlisting[language=C]{guide.c}
\end{minipage}
...
\begin{minipage}{\textwidth}
\lstinputlisting[language=C]{assign_expression.c}
\end{minipage}

It is straightforward to observe that this program has a polynomial growth bound.
The precise value of that bound is \pr|y1'|\symbo{Xprime} = \pr|y1| ∧ \pr|y2'|\symbo{Xprime} ≤ 2 * \pr|y1|.
...
The bound obtained by the analyzer is \pr|y1'|\symbo{Xprime} ≤ \pr|y1| \(\land\) \pr|y2'|\symbo{Xprime} ≤ \pr|y1|.
...
\begin{itemize}
\item \pr|y1'|\symbo{Xprime} ≤ \pr|y1| \(\land\) \pr|y2'| ≤ \(\max(0,0) + y1\)
\item \pr|y1'|\symbo{Xprime} ≤ \pr|y1| \(\land\) \pr|y2'| ≤ \(\max(0,0) + y1\)
\item \pr|y1'|\symbo{Xprime} ≤ \pr|y1| \(\land\) \pr|y2'| ≤ \(\max(0,\prm{y1}) + 0\)
\end{itemize}
They all simplify to \pr|y1'| ≤ \pr|y1| \(\land\) \pr|y2'| ≤ \pr|y1|\symbo{Xprime}.
...
\begin{minipage}{\textwidth}
\lstinputlisting[language=C]{exponent2_og.c}
\end{minipage}
...
\begin{minipage}{\textwidth}
\lstinputlisting[language=C]{notinfinite3.c}
\end{minipage}
...
\begin{minipage}{\textwidth}
\lstinputlisting[language=C]{infinite3.c}
\end{minipage}
...
\begin{minipage}{\textwidth}
\lstinputlisting[language=C]{dense_loop_og.c}
\end{minipage}
