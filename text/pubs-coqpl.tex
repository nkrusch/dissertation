...
formally for several reasons. In general, deriving a \ndx{complexity bound} for
an arbitrary program is an undecidable\index{undecidability} problem. In the
area of \ndx{complexity theory}, \textcquote[p.~114]{forster2020}{formalisations
of even basic complexity-theoretic results are not available}, hindering
...
will do so in polynomial time\index{time complexity} in the size of its inputs.
...
operations, \ndx{semi-ring}s, and \ndx{honest polynomial}s\footnote{Which are
\textcquote[p.~5]{jones2009}{polynomial build up from constants in
\(\mathbb{N}\)\symbo{nat} and variables by applying the operations \(+\)
(addition) and \(\times\) (multiplication).}} that are needed to represent the
\emph{mwp}-bounds\index{mwp-bound}. The \ndx{Mathematical Components}
library~\cite{mahboubi2022,mathcomp} will lay the foundations for the
\ndx{linear algebra} representations, but likely requires extensions to
