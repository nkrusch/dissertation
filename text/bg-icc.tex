...
Which particular model of computation\index{machine model} is used does not matter, as long as one adheres to Slot and van Emde Boas Invariance Thesis\index{Invariance Thesis}~\cite{slot1984}\footnote{
    The Invariance Thesis\index{Invariance Thesis} asserts that a time (resp., space) cost model is reasonable for a computational model C\index{machine model} if there are mutual simulations between TMs and C such that the overhead is polynomial in time\index{complexity class!P} (resp., linear in space)~\cite{vanoni2022}.},
...
For example, polynomial time complexity\index{complexity class!P} has been thoroughly examined under these terms.
...
Another common feature is that the characterizations may omit references to explicit resource bounds~\cite{moyen2017} -- refer to \eg~\textcite{bellantoni1992} or~\textcite{kristiansen2005} for examples.
...
\begin{center}
    ...
    \pr|exp(n + 1) = add(exp(n), exp(n))|.
\end{center}

\noindent Composing \pr|exp| with itself allows us to form towers of exponentials of arbitrary size. %, thus saturating the Kalmar elementary functions.
A recursion on \pr|exp| leads us even beyond the class of elementary recursive functions\index{recursive function}; %aforementioned class,
...
This makes \pr|exp| not expressible anymore, giving rise to new function algebra called \emph{safe recursion}\index{safe recursion}, discovered by Bellantoni and Cook~\cite{bellantoni1992} (BC in the following).

\begin{theorem}
    The class BC equals the class FP of polynomial time computable functions.
\end{theorem}\index{complexity class!P}
...
\begin{tabularx}{\textwidth}{@{}lclXr@{}}
    ...
    \endfoot
    \endlastfoot
    1992\(^*\) & \syntax & Comput  & Safe recursion\index{safe recursion} -- \autoref{safe-rec} & \ccx{p}\index{complexity class!P} \\*
    && Complex. & \textcite{bellantoni1992} \\*
    \midrule
    1993\(^*\) & \recs & POPL & Stratified recurrence\index{stratified recurrence} & \ccx{p}\index{complexity class!P} \\
    &&& \textcite{leivant1993} \\*
    \midrule
    1995 & \lalg & CSL & Tiering \aka ramification\index{tiering}\index{ramification} & \ccx{pspace}\index{complexity class!PSPACE} \\*
    &&& \textcite{leivant1995} \\*
    \midrule
    1999 & \types & LICS & Non-size-increasing programs\index{non-size-increasing programs} & \ccx{p}\index{complexity class!P} \\*
    &&& \textcite{hofmann1999} \\*
    \midrule
    2000 & \recs & LPAR & Quasi-interpretations\index{quasi-interpretation} & \ccx{p}\index{complexity class!P} \\*
    &&& \textcite{marion2000} \\*
    \midrule
    2001 & \syntax & J. Funct. & CONS-free programs\index{CONS-free programs} & \ccx{p}\index{complexity class!P}, \ccx{exp}\index{complexity class!EXP} \\*
    && Program. & \textcite{jones2001} & \\*
    \midrule
    2001 & \dataflow & POPL & Size-change termination principle\index{size-change termination principle} & \ccx{pspace}\index{complexity class!PSPACE} \\*
    &&& \textcite{lee2001} \\*
    \midrule
    2005 & \lalg & Comput. & \textsc{loop}$^{-}$ and \textsc{clip} programs & \ccx{l}\index{complexity class!L}, \ccx{linspace}\index{complexity class!LINSPACE} \\*
    && Complex. & \textcite{kristiansen2005} \\*
    \midrule
    2009 & \logics & TOCL & Flow calculus of mwp-bounds\index{mwp-calculus} -- \autoref{flow-calculus} & \ccx{p}\index{complexity class!P} \\*
    &&& \textcite{jones2009}  \\*
    \midrule
    2011 & \types & LICS & SAFE programs\index{SAFE programs} -- \autoref{icc-sec} & \ccx{p}\index{complexity class!P} \\*
    &&& \textcite{marion2011} \\*
    \midrule
    2013 & \types & ICALP & Ramified programs\index{ramification}\index{ramified programs} -- \autoref{icc-sec}  & \ccx{p}\index{complexity class!P}, \ccx{l}\index{complexity class!L} \\*
    &&& \textcite{leivant2013} \\*
    \midrule
    2015 & \types & LPAR & d$\ell$T\index{d\(\ell\)T} programs$^a$ -- \autoref{icc-sec} & \ccx{p}\index{complexity class!P} \\*
    &&& \textcite{baillot2015} \\*
    \midrule
    2016 & \logics & Inf. & Complexity with pointer machines & \ccx{l}\index{complexity class!L} \\*
    &&  Comput. & \textcite{aubert2016}  \\*
    \midrule
    2016 & \recs & Inf. & Complexity of uniform boolean circuits & \ccx{nc}\index{complexity class!NC} \\*
    && Comput. &\textcite{bonfante2016} \\*
    \midrule
    2018 & \types & Inf & Object Oriented programs\index{SAFE programs!object-oriented} -- \autoref{icc-sec} & \ccx{p}\index{complexity class!P} \\*
    && Comput. & \textcite{hainry2018} \\*
    \midrule
    2021 & \lalg & MFCS & Probabilistic characterization \(\text{ST}_\text{PP}\) & \ccx{pp}\index{complexity class!PP} \\*
    &&& \textcite{dallago2021} \\*
    \midrule
    2023 & \types & POPL & Stratified (STR) programs\index{stratified programs} -- \autoref{icc-sec} & \ccx{p}\index{complexity class!P} \\*
    &&& \textcite{hainry2023} \\*
    \midrule
    2024 & \types & POPL & Quantitative Type Theory (QTT)\index{quantitative type theory} & \ccx{p}\index{complexity class!P}, \ccx{np}\index{complexity class!NP}, \ccx{bpp}\index{complexity class!BPP} \\*
    &&& with dependent types\index{dependent types}, \textcite{atkey2024} \\*
    \midrule
    2024 & \types & LICS & Aperiodic programs\index{aperiodic programs} -- \autoref{icc-sec} & \ccx{p}\index{complexity class!P} \\*
    &&& \textcite{hainry2024} \\*
    \bottomrule
    \multicolumn{5}{@{}l}{$^a$ The system can handle sub-computations not in \ccx{p}.} \\
    \caption[Theoretical implicit complexity systems and results.]{
        A list of theoretical implicit complexity systems and results.
