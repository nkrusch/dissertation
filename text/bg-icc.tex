...
However, computability is independent of the required computational resources, \ie \emph{cost} or \emph{resource usage}\index{resource usage} of computation.
A computable function that requires an excessive amount of resources may not be solvable in practice.

Computational complexity theory\index{complexity theory} enhances the study of computable functions\index{computable function} by classifying them into \emph{complexity classes}\index{complexity classes}\footnote{
...
and is concerned with complexity classes\index{complexity classes}, \ie, order of magnitudes in the resources needed.
...
In this view, the unifying factor of implicit complexity is to find natural characterizations of complexity classes\index{complexity classes} through \emph{restrictions} embedded in \emph{programming languages}~\cite{hoffmann2022}.
