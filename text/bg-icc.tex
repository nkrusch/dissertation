...
see~\textcite{complexityzoo}.} based on resource requirements\index{resource
...
one adheres to Slot and van Emde Boas \ndx{Invariance Thesis}\footnote{The
\ndx{Invariance Thesis} asserts that a time (\resp space) cost model is
reasonable for a computational model \(\mathcal{C}\)\symbo{cmodel}\index{machine
model} if there are mutual simulations between TMs and \(\mathcal{C}\) such that
the overhead is polynomial in time\ccxi{p} (resp., linear in
space)~\cite{vanoni2022}.}~\cite{slot1984}, and is concerned with
\ndx{complexity classes}, \ie order of magnitudes in the resources needed. The
resource cost\index{resource usage} is estimated relative to the input size.
This way algorithms are allowed to consume more resources when the input size
grows while maintaining membership in the same complexity class\index{complexity
classes}. The common resources of interest are time\index{time complexity} and
space (memory)\index{space complexity}. Complexity measures\index{complexity
measure} are defined explicitly relative to the \ndx{machine model}. For
example, \ndx{time complexity} refers to the maximum number of computation steps
a machine\index{machine model} uses on any input~\cite[p. 276]{sipser2012}.
...
\ndx{asymptotic analysis} develops insight of costs\index{resource usage} of
computational patterns and identifies relationships between \ndx{complexity
classes}. For algorithms, it produces measures of efficiency, and allows
comparing relative performance of alternative algorithms~\cite{cormen2009}.

This overview of the classical \ndx{complexity theory} should be familiar from
...
complements the classical \ndx{complexity theory} by offering alternative
perspectives and systems for reasoning about cost of computation\index{resource
usage}.
...
Implicit computational complexity sees \ndx{complexity bound}s as consequences
...
class\index{complexity classes}, complexity analysis becomes a task of
...
\item The characterizations demonstrate that \ndx{complexity classes} are
...
potential to convert complexity-theoretic\index{complexity theory} insights to
practical analyses.
...
systems have been developed based on, or inspired by, \eg calculi, \ndx{term
rewriting}, \ndx{data flow analysis}, \ndx{primitive recursion}, \ndx{linear
logic}, \ndx{modal logic}, \ndx{type system}s, \ndx{digital circuit}s, and
...
restriction is maintained, the \ndx{complexity bound} will also be maintained.

The programming language-based approach has minimally two significant effects
compared to classical \ndx{complexity theory}. The first is a
...
embedding into a language a restriction by which a \ndx{complexity bound} will be
