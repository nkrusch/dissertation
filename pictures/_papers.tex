...
\path (1,18.5) node[bllet, conce, dotted, line width=1.2pt] {}
...
\path     (8, 17) node[start, conce] (icc) {\textbf{Implicit Computational Complexity}}
   ...
   ++(-1*\sx,  0) node[block=nil:\iconSEC:{A Logic for\\Anytime Non-Interference$^{\displaystyle *}$}:{\authD}]
   ++(   \sx,\sy) node[block=pci:\iconFM:{Polynomial Postconditions\\via mwp-Bounds$^{\displaystyle *}$}:{\authE}]
   ++(     0,\sy) node[block=cca:\iconFM:{Certifying Complexity Analysis$^{\displaystyle *}$}:{\authB (2023)\\\authE{ }(2023 -- ?)}];
...
\path (12, -1*\sy) node[text width=3cm] {Related Topics}
